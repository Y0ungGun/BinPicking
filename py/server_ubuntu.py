...
# Ubuntu-compatible path handling
script_dir = os.path.dirname(os.path.abspath(__file__))
os.chdir(script_dir)
print(f"Working directory: {os.getcwd()}")

# ONNX model path - use relative path for Ubuntu compatibility
onnx_model_path = "best.onnx"
if not os.path.exists(onnx_model_path):
    print(f"Warning: ONNX model file '{onnx_model_path}' not found in {os.getcwd()}")
    print("Please ensure the model file is in the correct location.")

# Initialize ONNX session with CUDA support if available, fallback to CPU
...
def get_next_data_no():
    ...
    pattern = os.path.join(save_dir, "*_*_*.png")
...
def run_graspability_model(instance_id, img_array):
    ...
    if cropped_objects:
        ...
        # Return dummy response when no objects detected
        dummy_feature = [0.0] * 256
        response = struct.pack('I', 256)
        response += struct.pack('256f', *dummy_feature)
        response += struct.pack('2f', 736//2, 736//2)
        return response
...
# 서버 설정 - Ubuntu compatible
HOST = '0.0.0.0'  # Listen on all interfaces for Ubuntu server
PORT = 7779
IMAGE_PATH = "./images"
NUM_WORKERS = 1
...
encoder_path = "encoder/rn256/encoder_rn256.pth"
if os.path.exists(encoder_path):
    ...
    print(f"Warning: Encoder file '{encoder_path}' not found. Using random initialization.")

# Load output checkpoint if available
...
if os.path.exists(output_ckpt):
    try:
        ...
        print(f"Warning: Could not load output checkpoint: {e}")
...
# Metrics tracking
concurrent_clients = Semaphore(32)  # Limit to 32 concurrent clients
