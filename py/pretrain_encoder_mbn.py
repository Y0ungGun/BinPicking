...
# 2. AutoEncoder 정의 (Encoder는 MobileNetV3-Small 사용, Decoder만 학습)
class Encoder(nn.Module):
    def __init__(self, feature_dim=128):
        ...
        self.fc = nn.Linear(576, feature_dim)  # MobileNetV3-Small 마지막 채널 수는 576
        # Freeze only MobileNet features
...
class Decoder(nn.Module):
    def __init__(self, feature_dim=128, img_size=120):
        super().__init__()
        self.fc = nn.Linear(feature_dim, 576)
        self.img_size = img_size
        self.deconv = nn.Sequential(
            nn.Unflatten(1, (576, 1, 1)),
            nn.ConvTranspose2d(576, 256, 4),
            nn.ReLU(),
